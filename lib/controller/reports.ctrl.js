...
module.exports = {
    ...
    getVendorPaymentReport: function(req, res)
    {
        if (req.decoded != undefined && req.decoded.success == true) {   
            connection.acquire(function(err, con){

                con.query('SELECT vendors.id, vendors.name,IFNULL((SELECT SUM(pomaster.netamt) FROM pomaster WHERE pomaster.vendorid = vendors.id),0) AS total_order_amout, (IFNULL((SELECT SUM(vendorpayment.paidamt) FROM vendorpayment WHERE vendorpayment.vendorid = vendors.id),0)) AS total_paid_amount, (SELECT SUM(pomaster.pendingpayment) FROM `pomaster` WHERE `vendorid` = vendors.id) AS total_pending_amount FROM vendors WHERE vendors.companyid = '+req.decoded.logedinuser.companyid+' HAVING total_order_amout > 0', function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:0, message:'Something went wrong, Please try again.'});
    ...
    getVendorsPendingPaymentsData: function(req, res)
    {
        if (req.decoded != undefined && req.decoded.success == true) {   
            connection.acquire(function(err, con){

                con.query('SELECT pomaster.id, pomaster.orderdate,(SELECT COUNT(*) FROM podetails WHERE podetails.poorderid = pomaster.id) as items,pomaster.pendingpayment FROM pomaster WHERE pomaster.vendorid = '+req.body.vendor_id+' AND pendingpayment > 0', function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:0, message:'Something went wrong, Please try again.'});
    ...
    getPurchasedItemsList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded.success == true) {   
            connection.acquire(function(err, con){

                con.query('SELECT `id`, (SELECT products.name FROM products WHERE products.id = podetails.productid) as name, `qty`,  `price`,`unit`, `netprice` FROM `podetails` WHERE `poorderid` =  '+req.body.purchase_id, function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:0, message:'Something went wrong, Please try again.'});
                    ...
                    {
                        res.send({purchaseItemsList:result});
                    }
                    con.release();
                });
