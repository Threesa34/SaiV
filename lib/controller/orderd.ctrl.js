...
module.exports = {
    ...
    deletePurchaseOrder: function(req, res)
    {
       if (req.decoded != undefined && req.decoded.success == true) {    
            connection.acquire(function(err, con){
                con.query('DELETE FROM `podetails` WHERE `poorderid` = '+req.params.id+'; DELETE FROM `pomaster` WHERE `id` = '+req.params.id+';', function(err, result){
                    if(err)
                    {
                        res.send({
                            status: 1,
    ...
    SaveVendorPaymentCollection : function(req, res)
    {
        if (req.decoded != undefined && req.decoded.success == true) {    
            var sql = '';
            var values = '';
            if(req.body.length > 0)
            {
                req.body.map(function(value){
                    ...
                    sql = sql+"UPDATE `pomaster` SET `pendingpayment` = "+value.pending_amount+" WHERE `id` = "+value.id+";";

                    values = values + "("+value.id+","+value.vendorid+","+value.paid_amount+","+value.pending_amount+",'"+moment(value.payment_date).format('YYYY-MM-DD HH:mm:ss')+"','"+value.paid_by+"','"+value.payment_mode+"','"+value.txnno+"','"+value.Txndate+"','"+value.bank+"',"+req.decoded.logedinuser.id+","+req.decoded.logedinuser.companyid+"),";
             ...
             console.log(sql+'INSERT INTO `vendorpayment`(`orderid`, `vendorid`, `paidamt`, `pendingamt`, `paymentdate`, `receivedby`, `paymentmode`,`txnno`, `txndate`, `bank`, `createdby`, `companyid`) VALUES'+ values);

             connection.acquire(function(err, con){
                con.query(sql+'INSERT INTO `vendorpayment`(`orderid`, `vendorid`, `paidamt`, `pendingamt`, `paymentdate`, `receivedby`, `paymentmode`,`txnno`, `txndate`, `bank`, `createdby`, `companyid`) VALUES'+ values, function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
    ...
    getAllInvoicesCopies: function(req, res)
    {
       if (req.decoded != undefined && req.decoded.success == true) {    
            console.log(req.body)
            const dir = './app/invoices/';
            var files = fs.readdirSync(dir)
            if(files != undefined && files.length > 0)
            {
                var selectedfiles = files.filter(function(value){
                    return value.includes(req.body.name);
                });
                if(selectedfiles != undefined && selectedfiles.length > 0)
                {
                    res.send({filesList:selectedfiles})
                }
                else
                {
                    res.send({filesList:[]})
    ...
    savePurchaseOrderDetails: function(req, res)
    {
       if (req.decoded != undefined && req.decoded.success == true) {   
            ...
            connection.acquire(function(err, con){
                ...
                con.query('INSERT INTO `pomaster`(`vendorid`, `orderdate`,`netamt` ,`createdby`, `pendingpayment`,`companyid`) VALUES (?,"'+moment(req.body[0].orderdate).format("YYYY-MM-DD HH:mm:ss")+'",?,?,?,?)',[req.body[0].vendorid,req.body[0].netamt,req.decoded.logedinuser.id, req.body[0].netamt, req.decoded.logedinuser.companyid], function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
